...
describe("FacilitiesAndReservationsContext", () => {
    ...
    let testDate; // Variable para almacenar la fecha para el test de updateReservation

    beforeEach(() => {
        jest.clearAllMocks();
        facilitiesReservationsValues = null;
        useFacilitiesAndReservations.mockReturnValue(mockFacilitiesAndReservationsContext);
        testDate = new Date(); // Inicializa la fecha en beforeEach
    ...
    it("should add a new reservation", async () => {
        const response = await facilitiesReservationsValues.addReservation({ instalacionId: '1', fechaInicio: new Date() });
    ...
    it("should add a new facility", async () => {
        ...
        expect(response.name).toBe('Pista de Tenis'); // Mock returns the new facility
